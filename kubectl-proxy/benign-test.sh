...
for((i=1; i<=repeat_count; i++)); do
    ...
    if [ "$response" -eq 200 ]; then
        echo "Request successful. Proceeding to the next step."
        # 다음 명령어 실행
        # next_command_here
    else
        echo "Request failed or timed out (HTTP status: $response)."
    ...
    GRAPHS_DIR="./graphs/graphs_benign"
    GRAPHS_DOT_DIR="./graphs/graphs_benign_dot"
